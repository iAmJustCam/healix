...
export async function validateTypeScript5Implementation(
  ...
  const verbatimModuleSyntaxIssues =
    await validateVerbatimModuleSyntax(context);
  ...
  const filteredTsFiles = tsFiles.filter(
  ...
  for (const file of filteredTsFiles) {
    console.log(
      `Analyzing TypeScript implementation in: ${path.relative(context.rootDir, file)}`,
    );
    ...
    const exactOptionalPropertyIssues = validateExactOptionalPropertyTypes(
      parsedFile,
      file,
    );
 ...
 */
function validatePropInterfaces(
  parsedFile: any,
  filePath: string,
): ValidationIssue[] {
  const issues: ValidationIssue[] = [];
  ...
  if (
    parsedFile.hasPropInterface &&
    !parsedFile.propInterfaceName.endsWith("Props")
  ) {
    issues.push({
      type: "warning",
 ...
 */
function validateContextTyping(
  parsedFile: any,
  filePath: string,
): ValidationIssue[] {
  const issues: ValidationIssue[] = [];
 ...
 */
function validateAdvancedPatterns(
  parsedFile: any,
  filePath: string,
): ValidationIssue[] {
  const issues: ValidationIssue[] = [];
  ...
  if (parsedFile.hasInterfacePropsWithoutJSDoc) {
    issues.push({
      ...
      fix: {
        ...
        steps: ["Add /** Description */ comments above each property"],
      },
      documentation: "https://tsdoc.org/",
    });
  }
 ...
 */
function validateConstTypeParameters(
  parsedFile: any,
  filePath: string,
): ValidationIssue[] {
  const issues: ValidationIssue[] = [];
  ...
  if (parsedFile.couldUseConstTypeParams) {
    issues.push({
      type: "suggestion",
      message:
        "Consider using const type parameters for improved type inference",
      filePath,
 ...
 */
async function validateVerbatimModuleSyntax(
  context: ProjectContext,
): Promise<ValidationIssue[]> {
  const issues: ValidationIssue[] = [];
 ...
 */
async function validateSatisfiesOperator(
  context: ProjectContext,
): Promise<ValidationIssue[]> {
  const issues: ValidationIssue[] = [];
 ...
 */
async function validateTypeOnlyImports(
  context: ProjectContext,
): Promise<ValidationIssue[]> {
  const issues: ValidationIssue[] = [];
 ...
 */
function validateExactOptionalPropertyTypes(
  parsedFile: any,
  filePath: string,
): ValidationIssue[] {
  const issues: ValidationIssue[] = [];
  ...
  if (parsedFile.hasInterfaceWithOptionalProps) {
    ...
    issues.push({
      type: "suggestion",
      message:
        "Consider enabling exactOptionalPropertyTypes in tsconfig.json for stricter optional property checking",
      filePath,
...
export function convertToSatisfiesOperator(
  ...
  if (colonIndex > 0 && equalsIndex > colonIndex) {
    ...
    const typeAnnotation = variableLine
      .substring(colonIndex + 1, equalsIndex)
      .trim();
