 ...
 */
async function validateAppRouter(
  context: ProjectContext,
  ...
  const hasAppDirectory = await directoryExists(
    path.join(context.rootDir, "src", "app"),
  );
  const hasPagesDirectory = await directoryExists(
    path.join(context.rootDir, "src", "pages"),
  );
...
body {
  ...
  if (hasAppDirectory) {
    ...
    const allRoutes = [
      ...routeAnalysis.appRoutes.map((route) => ({
        ...route,
        depth: route.path.split("/").length,
      })),
      ...routeAnalysis.pageRoutes.map((route) => ({
        ...route,
        depth: route.path.split("/").length,
      })),
    ];
    const deepRoutes = allRoutes.filter(
      (route: { path: string; depth: number }) => route.depth > 3,
    );
...
async function validateMetadata(
  ...
  for (const pagePath of pageRoutes) {
    ...
    if (
      content.content.includes("export const metadata") ||
      content.content.includes("export async function generateMetadata")
    ) {
      ...
      if (metadataVariable) {
        ...
        if (initializer && Node.isObjectLiteralExpression(initializer)) {
          ...
          const hasTitle = properties.some(
            (prop) =>
              Node.isPropertyAssignment(prop) && prop.getName() === "title",
          );
          ...
          const hasOpenGraph = properties.some(
            (prop) =>
              Node.isPropertyAssignment(prop) && prop.getName() === "openGraph",
          );
  ...
  images: [{ url: '/og-image.jpg', width: 1200, height: 630 }]
              ...
              documentation:
                "https://nextjs.org/docs/app/building-your-application/optimizing/metadata#openGraph",
            });
          }
        }
      }
    ...
    issues.push({
      ...
      fix: {
        ...
        context: {
          title: path
            .basename(path.dirname(pagePath))
            .replace(/[-_]/g, " ")
 ...
 */
async function validateRouteHandlers(
  context: ProjectContext,
): Promise<ValidationIssue[]> {
  const issues: ValidationIssue[] = [];
  ...
  for (const routePath of routeHandlers) {
    ...
    if (
      !content.content.includes("export async function GET") &&
    ...
    if (!content.content.includes("NextResponse")) {
      issues.push({
        ...
        documentation:
          "https://nextjs.org/docs/app/building-your-application/routing/route-handlers#response",
      });
    }
  }
 ...
 */
async function validateDataFetching(
  context: ProjectContext,
): Promise<ValidationIssue[]> {
  const issues: ValidationIssue[] = [];
  ...
  const allFiles = await getFilesWithExtension(srcPath, [
    ".tsx",
    ".jsx",
    ".ts",
  ]);
  ...
  for (const filePath of tsxFiles) {
    ...
    if (
      content.content.includes("useSWR(") &&
      !content.content.includes("revalidate")
    ) {
      issues.push({
        type: "suggestion",
    ...
    if (
      content.content.includes("export async function getServerSideProps") ||
      content.content.includes("export async function getStaticProps")
    ) {
      issues.push({
        type: "error",
        message:
          "Using Pages Router data fetching methods instead of App Router patterns",
        filePath,
        line: 0,
        ...
        documentation:
          "https://nextjs.org/docs/app/building-your-application/upgrading/app-router-migration#step-4-migrating-data-fetching-methods",
      });
    }
  }
...
module.exports = nextConfig
  ...
  if (
    !configContent.content.includes("serverActions") ||
    !configContent.content.includes("true")
  ) {
    issues.push({
      type: "warning",
  ...
  if (
    configContent.content.includes("webpack(config)") &&
    configContent.content.includes("optimization")
  ) {
    issues.push({
      type: "suggestion",
      message:
        "Manual webpack optimizations may conflict with Next.js defaults",
      filePath: configPath,
      line: 0,
...
async function validateMiddleware(
  ...
  if (
    !content.content.includes("export function middleware") &&
    !content.content.includes("export default function")
  ) {
    issues.push({
      type: "error",
 ...
 */
async function validateAsyncLayoutAPIs(
  context: ProjectContext,
): Promise<ValidationIssue[]> {
  const issues: ValidationIssue[] = [];
  ...
  const layouts = layoutFiles.filter(
    (file) => path.basename(file) === "layout.tsx",
  );
  ...
  for (const layoutPath of layouts) {
    ...
    for (const func of functions) {
      ...
      const usesCookiesOrHeaders =
        functionText.includes("cookies()") ||
 ...
 */
async function validateDynamicRouteSegments(
  context: ProjectContext,
): Promise<ValidationIssue[]> {
  const issues: ValidationIssue[] = [];
    ...
    for (const entry of entries) {
      if (entry.isDirectory()) {
        ...
        if (
          ...
          (!segmentName.includes("]]") ||
            segmentName.indexOf("]]") !== segmentName.length - 2)
        ) {
          issues.push({
            type: "error",
        ...
        if (
          segmentName.includes("[...") &&
          (!segmentName.includes("]") ||
            segmentName.indexOf("]") !== segmentName.length - 1)
        ) {
          issues.push({
            type: "error",
 ...
 */
async function validateGenerateStaticParams(
  context: ProjectContext,
): Promise<ValidationIssue[]> {
  const issues: ValidationIssue[] = [];
  ...
  const dynamicRouteFiles = tsxFiles.filter((file) => {
    ...
    return (
      (dirName.startsWith("[") || dirName.includes("[...")) &&
    ...
    if (!staticParamsFunc) {
      ...
      if (path.basename(filePath) === "page.tsx") {
        issues.push({
          type: "suggestion",
          message:
            "Dynamic route segment page should implement generateStaticParams",
          filePath,
          line: 0,
 ...
 */
async function validateLegacyApiInAppRouter(
  context: ProjectContext,
): Promise<ValidationIssue[]> {
  const issues: ValidationIssue[] = [];
  ...
  for (const filePath of tsxFiles) {
    ...
    if (getStaticPaths) {
      issues.push({
        type: "error",
        message:
          "getStaticPaths is not supported in App Router - use generateStaticParams instead",
        filePath,
    ...
    const hasGetInitialProps = sourceFile
      .getText()
  ...
  for (const filePath of tsxFiles) {
    ...
    for (const element of jsxElements) {
      ...
      if (tagName === "img") {
        ...
        const hasNextImageImport = sourceFile
          .getImportDeclarations()
          .some(
            (importDecl) =>
              importDecl.getModuleSpecifierValue() === "next/image",
          );
