...
module.exports = {
  ...
  const hasContentConfig =
    await checkTailwindConfigForContent(tailwindConfigPath);
  const hasThemeExtend =
    await checkTailwindConfigForThemeExtend(tailwindConfigPath);
  const hasRecommendedPlugins =
    await checkTailwindConfigForPlugins(tailwindConfigPath);
  const hasDarkModeConfig =
    await checkTailwindConfigForDarkMode(tailwindConfigPath);
  const overridesDefaultColors =
    await checkTailwindConfigForColorOverrides(tailwindConfigPath);
 ...
 */
async function checkTailwindConfigForContent(
  configPath: string,
): Promise<boolean> {
 ...
 */
async function checkTailwindConfigForThemeExtend(
  configPath: string,
): Promise<boolean> {
  const content = await fs.promises.readFile(configPath, "utf8");
 ...
 */
async function checkTailwindConfigForPlugins(
  configPath: string,
): Promise<boolean> {
  const content = await fs.promises.readFile(configPath, "utf8");
  return (
    content.includes("@tailwindcss/forms") ||
    content.includes("@tailwindcss/typography")
  );
}
 ...
 */
async function checkTailwindConfigForDarkMode(
  configPath: string,
): Promise<boolean> {
  const content = await fs.promises.readFile(configPath, "utf8");
 ...
 */
async function checkTailwindConfigForColorOverrides(
  configPath: string,
): Promise<boolean> {
  const content = await fs.promises.readFile(configPath, "utf8");
  return (
 ...
 */
async function validateCascadeLayers(
  context: ProjectContext,
): Promise<ValidationIssue[]> {
  const issues: ValidationIssue[] = [];
  ...
  if (!globalCssExists) {
    const srcGlobalCssPath = path.join(
      context.rootDir,
      "src",
      "styles",
      "globals.css",
 ...
 */
async function checkCascadeLayersInFile(
  filePath: string,
  ...
  if (content.includes("@tailwind") && !content.includes("@layer")) {
    issues.push({
      ...
      documentation:
        "https://tailwindcss.com/docs/adding-custom-styles#using-css-cascade-layers",
    });
  }
 ...
 */
async function validateClassUsage(
  context: ProjectContext,
): Promise<ValidationIssue[]> {
  const issues: ValidationIssue[] = [];
  ...
  for (const filePath of componentFiles) {
    ...
    if (
      classAnalysis.usesStringConcatenation &&
      !sourceFile.getFullText().includes("clsx") &&
      !sourceFile.getFullText().includes("cn(")
    ) {
      issues.push({
        type: "warning",
        message:
          "Use cn() or clsx() instead of string concatenation for className",
        filePath,
    ...
    if (
      classAnalysis.usesTemplateLiterals &&
      !sourceFile.getFullText().includes("clsx") &&
      !sourceFile.getFullText().includes("cn(")
    ) {
      issues.push({
        type: "warning",
        message:
          "Use cn() or clsx() instead of template literals for className",
        filePath,
    ...
    if (
      (classAnalysis.usesConditionalClasses ||
        classAnalysis.usesTemplateLiterals) &&
      !sourceFile.getFullText().includes("clsx") &&
      !sourceFile.getFullText().includes("cn(") &&
      cnUtilExists
  ...
  for (const attr of jsxAttributes) {
    ...
    if (attrName === "className") {
      ...
      if (Node.isStringLiteral(initializer)) {
        ...
        const colorMatches = classValue.match(
  ...
  for (const filePath of componentFiles) {
    ...
    for (const colorClass of classAnalysis.colorClasses) {
      ...
      const match = colorClass.match(
        /^(bg|text|border|ring|shadow|fill|stroke)-([\w-]+)(?:-(\d+))?$/,
      );
      if (match) {
        ...
        if (
          shade &&
          !["500", "600", "700", "900", "100", "200"].includes(shade)
        ) {
 ...
 */
async function validateUtilityHelpers(
  context: ProjectContext,
): Promise<ValidationIssue[]> {
  const issues: ValidationIssue[] = [];
  ...
  if (!cnUtilExists) {
    ...
    if (libDirExists) {
      ...
      for (const file of libFiles.allFiles) {
        ...
        if (
          (content.includes("clsx") || content.includes("classnames")) &&
          content.includes("twMerge")
        ) {
          found = true;
          break;
        }
  ...
  if (packageJsonExists) {
    try {
      ...
      if (
        ...
        !packageJson.devDependencies?.["clsx"]
      ) {
 ...
 */
async function validateLogicalProperties(
  context: ProjectContext,
): Promise<ValidationIssue[]> {
  const issues: ValidationIssue[] = [];
  ...
  for (const filePath of componentFiles) {
    ...
    for (const attr of jsxAttributes) {
      ...
      const directionalClasses = [
        ...
        {
          pattern: /\bborder-l-\d+\b/,
          logical: "border-s-",
          direction: "left border",
        },
        {
          pattern: /\bborder-r-\d+\b/,
          logical: "border-e-",
          direction: "right border",
        },
        {
          pattern: /\btext-left\b/,
          logical: "text-start",
          direction: "left text alignment",
        },
        {
          pattern: /\btext-right\b/,
          logical: "text-end",
          direction: "right text alignment",
        },
      ];
 ...
 */
async function validateCustomPluginMigration(
  context: ProjectContext,
): Promise<ValidationIssue[]> {
  const issues: ValidationIssue[] = [];
 ...
 */
async function validateDeprecatedUtilities(
  context: ProjectContext,
): Promise<ValidationIssue[]> {
  const issues: ValidationIssue[] = [];
  ...
  const deprecatedClasses = [
    {
      pattern: /\bcontent-\w+\b/,
      replacement: "contentUtil",
      description: "content utilities",
    },
    {
      pattern: /\btransform\b/,
      replacement: "transformUtil",
      description: "transform utility",
    },
    {
      pattern: /\bfilter\b/,
      replacement: "filterUtil",
      description: "filter utility",
    },
    {
      pattern: /\bisolation-\w+\b/,
      replacement: "isolationUtil",
      description: "isolation utility",
    },
  ];
 ...
 */
async function validateInlineStylesToTailwind(
  context: ProjectContext,
): Promise<ValidationIssue[]> {
  const issues: ValidationIssue[] = [];
  ...
  for (const filePath of componentFiles) {
    ...
    const styleAttributes = sourceFile
      .getDescendantsOfKind(SyntaxKind.JsxAttribute)
        ...
        if (convertibleProperties.length > 0) {
          issues.push({
            ...
            fix: {
              ...
              context: {
                line: attr.getStartLineNumber(),
                properties: convertibleProperties
                  .map((p) => {
