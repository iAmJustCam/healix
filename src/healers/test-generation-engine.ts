 ...
 */
export function getExportableEntityKind(sourceFile: SourceFile): {
  ...
  for (const [name, declarations] of exportedElements) {
    ...
    for (const declaration of declarations) {
      const declarationText = declaration.getText();
      if (
        (declarationText.includes("function use") ||
          declarationText.includes("const use")) &&
...
export async function generateComponentTests(
  ...
  const allFiles = await getFilesWithExtension(
    componentsDir,
    [".tsx", ".jsx", ".js"],
    true,
  );
  ...
  if (options.componentName) {
    ...
    if (filteredComponents.length > 0) {
      console.log(
        `Filtering to ${filteredComponents.length} components matching "${options.componentName}"`,
      );
...
async function generateTestContent(
  ...
  const needsUseMock =
    componentContent.includes(" use(") ||
    componentContent.includes(".then") ||
  ...
  const hasTestUtils = await fileExists(
    path.join(path.dirname(componentPath), "..", "utils", "test-utils.ts"),
  );
  if (hasTestUtils) {
  ...
  if (
    componentContent.includes("useGame") ||
    componentContent.includes("useUser")
  ) {
...
vi.mock('react', async () => {
  ...
  let fetchMock = "";
  if (hasFetch && !needsUseMock) {
    // Skip if we're already mocking use()
    fetchMock = `
// Mock fetch
global.fetch = vi.fn().mockResolvedValue({
  ...
  const typedProps: PropInfo[] =
    props && Array.isArray(props)
      ? props.map((p) =>
          typeof p === "string"
            ? { name: p, type: guessTypeFromName(p) }
            : (p as PropInfo),
        )
      : [];
  ...
  if (isInteractive) {
    ...
    if (
      componentContent.includes("onClick") ||
      componentContent.includes("handleClick")
    ) {
      testCases.push(`
  it('handles click events correctly', async () => {
    ...
    if (
      componentContent.includes("onChange") ||
      componentContent.includes("handleChange")
    ) {
      testCases.push(`
...
async function generateComponentTestContent(
  ...
  const needsUserEvent =
    componentContext.content.includes("onClick") ||
    componentContext.content.includes("onChange") ||
    componentContext.content.includes("onSubmit");

  const needsTestUtils =
    componentContext.content.includes("use(") ||
  ...
  if (
    componentContext.content.includes("useGame") ||
    componentContext.content.includes("useUser")
  ) {
    imports.push(`
// Mocking hooks
vi.mock('@/hooks', () => ({
...
${propsText}
  ...
  if (
    componentContext.content.includes("onClick") ||
    componentContext.content.includes("handleClick")
  ) {
    testCases.push(`
 ...
 */
async function generateHookTestContent(
  hookPath: string,
  hookName: string,
): Promise<string> {
  ...
  const usesFetch =
    hookContent.includes("fetch(") || hookContent.includes("axios.");
 ...
 */
async function generateUtilityTestContent(
  utilPath: string,
  utilName: string,
): Promise<string> {
  // Create a relative import path
  ...
  const paramMatch = utilContent.match(
    new RegExp(
      `(function\\s+${utilName}|const\\s+${utilName}\\s*=\\s*(?:function)?\\s*)\\((.*?)\\)`,
      "s",
    ),
  );
  const params =
    paramMatch && paramMatch[2]
  ...
  if (
    utilContent.includes("map") ||
    utilContent.includes("filter") ||
    utilContent.includes("reduce")
  ) {
    testCases.push(`
...
function generatePropMocks(
  props: PropInfo[] | string[],
  componentContent: string,
): { mockDefinitions: string; propsJsx: string } {
  // If no props specified, try to extract from content
  const extractedProps =
    props.length === 0 ? extractPropsFromContent(componentContent) : props;
...
const mockProps = {
  ${extractedProps
    .map((prop) => {
      ...
      const propType =
        typeof prop === "string" ? guessTypeFromName(prop) : prop.type;
...
function generateBasicAssertions(
  context: ComponentContext,
  componentContent: string,
): string {
  const assertions = [];
  ...
  // Check for headings
  if (
    componentContent.includes("<h1") ||
    componentContent.includes("<h2") ||
    componentContent.includes("<h3") ||
    componentContent.includes("<h4")
  ) {
  ...
  if (
    ...
    componentContent.includes('role="button"')
  ) {
  ...
  if (
    ...
    componentContent.includes('role="img"')
  ) {
  ...
  if (componentContent.includes("data-testid=")) {
    ...
    if (testIdMatch) {
      assertions.push(
        `expect(screen.getByTestId('${testIdMatch[1]}')).toBeInTheDocument();`,
      );
    }
  }
