...
export async function healComponent(
  ...
  if (options.moveToSrc && !componentPath.includes("/src/")) {
    ...
    const newDir = path.join(
      "src",
      relativePath === "components" ? "components" : "components",
      parsedPath.name.toLowerCase(),
    );
    const newPath = path.join(
      newDir,
      parsedPath.name.toLowerCase() + parsedPath.ext,
    );
  ...
  if (issuesByFramework.nextjs) {
    ...
    const useClientIssues = issuesByFramework.nextjs.filter(
      (issue) =>
        issue.code === "NEXTJS_MISSING_USE_CLIENT" ||
        issue.code === "REACT19_MISSING_USE_CLIENT",
    );
  ...
  try {
    const {
      validateTransformedCode,
    } = require("./component-healing-validator");
    validationIssues = validateTransformedCode(sourceFile, transformedContent);

    if (validationIssues.length > 0) {
      console.warn(
        `⚠️ Validation found ${validationIssues.length} issues with the transformed code`,
      );
      validationIssues.forEach((issue) => {
        console.warn(
          `  - ${issue.type}: ${issue.message} at line ${issue.line}`,
        );
      });
  ...
  const barrelIssues = await ensureBarrelExport(
    finalComponentPath,
    componentContext,
  );
  ...
  if (options.generateTests) {
    const testResult = await generateTestForComponent(
      finalComponentPath,
      componentContext,
      options.dryRun,
    );
...
function extractComponentContext(
  ...
  const context: ComponentContext = {
    ...
    componentName:
      componentDeclarations[0]?.getName() ||
      path.basename(filePath).split(".")[0],

    // Determine component type based on declaration
    isArrowFunction: componentDeclarations.some(
      (c) =>
        Node.isVariableDeclaration(c) &&
        c
          .getFirstAncestorByKind(SyntaxKind.VariableDeclarationList)
          ?.getKind() === SyntaxKind.ConstKeyword,
    ),
    isFunctionDeclaration: componentDeclarations.some(
      (c) => Node.isMethodDeclaration(c) || Node.isFunctionDeclaration(c),
    ),

    // Check for React FC usage
    usesReactFC:
      sourceFile.getFullText().includes("React.FC") ||
      sourceFile.getFullText().includes("React.FunctionComponent"),

    // Check for props interface
    hasPropInterface: sourceFile
      .getInterfaces()
      .some((i) => i.getName().endsWith("Props")),
...
// Group issues by framework for ordered processing
function groupIssuesByFramework(
  issues: ValidationIssue[],
    ...
    switch (type) {
      case "replace":
        ...
        transformSourceFile(sourceFile, (sf) => {
          ...
          const options =
            typeof patternOptions === "string" ? patternOptions : undefined;
    ...
    switch (type) {
      case "replace":
        ...
        transformSourceFile(sourceFile, (sf) => {
          ...
          if (
            patternStr.includes("useEffect") &&
            issue.code === "REACT19_INCOMPLETE_EFFECT_DEPS" &&
            issue.fix
          ) {
            ...
            for (const call of callExpressions) {
              ...
              if (
                ...
                expression.getText() === "useEffect"
              ) {
                const args = call.getArguments();
                if (args.length === 1) {
                  // Only one argument - missing dependency array
                  const callback = args[0];
                  // Add the dependency array
                  const dependencies =
                    (issue.fix.dependencies as string[]) || [];
                  const depArray =
...
async function applyNextJsFixes(
  ...
  for (const issue of issues) {
    ...
    // Destructure fix object for safer access
    const { type, pattern, patternOptions, replacement, transformer } =
      issue.fix;
...
async function ensureBarrelExport(
  componentPath: string,
...
function addPropTypesToComponent(
  sourceFile: SourceFile,
  context: ComponentContext,
): boolean {
...
`.trim();
  ...
  const lastImportDeclaration = sourceFile
...
function addReactFCToComponent(
  sourceFile: SourceFile,
  context: ComponentContext,
): boolean {
      ...
      if (initializer && Node.isArrowFunction(initializer)) {
        ...
        if (variableDeclarationList) {
          ...
          const variableStatement =
            variableDeclarationList.getFirstAncestorByKind(
  ...
  if (
    ...
    if (reactImport) {
      ...
      if (
        reactImport.getNamedImports().length > 0 &&
        !reactImport.getDefaultImport()
      ) {
        const oldImport = reactImport.getText();
        const newImport = oldImport.replace("import {", "import React, {");
        sourceFile.replaceText(
          [reactImport.getStart(), reactImport.getEnd()],
          newImport,
        );
      }
    } else {
...
function transformToUseHook(
  ...
  const useEffectCalls = sourceFile
    ...
    .filter((call) => {
      const expression = call.getExpression();
      return (
        Node.isIdentifier(expression) && expression.getText() === "useEffect"
      );
    });

  for (const useEffectCall of useEffectCalls) {
    // Check if this useEffect contains a fetch call
    const fetchCalls = useEffectCall
      .getDescendantsOfKind(SyntaxKind.CallExpression)
      .filter((call) => {
        const expression = call.getExpression();
        return (
          Node.isIdentifier(expression) && expression.getText() === "fetch"
        );
      });
      ...
      if (arrowFunction && Node.isArrowFunction(arrowFunction)) {
        ...
        fetchCalls.forEach((fetchCall) => {
          ...
          if (fetchStatementText.includes(".then")) {
            ...
            const thenCall = fetchCall.getFirstDescendantByKind(
              SyntaxKind.PropertyAccessExpression,
            );
            const statementParent = fetchCall.getFirstAncestorByKind(
            ...
            if (thenCall && statementParent) {
              ...
              sourceFile.replaceText(
                [statementParent.getStart(), statementParent.getEnd()],
                useHookText,
              );
              hasChanges = true;

              // Also remove the dependency array and enclosing useEffect
              const useEffectStatement = useEffectCall.getFirstAncestorByKind(
                SyntaxKind.ExpressionStatement,
              );
  ...
  for (const axiosCall of axiosCalls) {
    if (axiosCall.getText().includes(".then")) {
      const statement = axiosCall.getFirstAncestorByKind(
        SyntaxKind.ExpressionStatement,
      );
      if (statement) {
        // Extract the URL
  ...
  if (hasChanges) {
    ...
    for (const reactImport of reactImports) {
      ...
      if (!hasUseImport) {
        ...
        const namedImportsText = namedImports
          .map((n) => n.getText())
          .join(", ");
        if (namedImportsText) {
          ...
          reactImport.replaceWithText(
            reactImport
              .getText()
              .replace(`{ ${namedImportsText} }`, newNamedImports),
          );
        } else {
...
function transformToNamedExport(
  sourceFile: SourceFile,
  context: ComponentContext,
): boolean {
  let hasChanges = false;
  ...
  try {
    ...
    if (exportDefaultMatch && exportDefaultMatch[1]) {
      ...
      const functionMatch = fullText.match(
      ...
      if (!exportAssignment.isExportEquals()) {
        ...
        if (Node.isIdentifier(expression)) {
          ...
          const varDeclaration =
            sourceFile.getVariableDeclaration(componentName);
          if (varDeclaration) {
            const varStatement = varDeclaration.getFirstAncestorByKind(
              SyntaxKind.VariableStatement,
            );
            if (varStatement) {
      ...
      if (match && match[1]) {
        ...
        if (functionPattern.test(newText)) {
          ...
          const varPattern = new RegExp(
            `(const|let|var)\\s+${componentName}\\s*=`,
          );
...
function transformToDestructuredProps(
  sourceFile: SourceFile,
  context: ComponentContext,
): boolean {
  ...
  if (hasChanges) {
    ...
    for (const propAccess of propertyAccesses) {
      ...
      if (Node.isIdentifier(expression) && expression.getText() === "props") {
        ...
        sourceFile.replaceText(
          [propAccess.getStart(), propAccess.getEnd()],
          propName,
        );
      }
    }
...
function transformToAppRouter(
  ...
  if (
    sourceFile.getFullText().includes("useState") ||
...
function replaceGetServerSideProps(
  sourceFile: SourceFile,
  context: ComponentContext,
): boolean {
  let hasChanges = false;
  ...
  const gsspFunction = sourceFile
    .getFunctions()
    .find((func) => func.getName() === "getServerSideProps");
  ...
  if (fetchCalls.length > 0) {
    ...
    if (componentDeclaration) {
      ...
      if (hasChanges) {
        ...
        for (const reactImport of reactImports) {
          const namedImports = reactImport.getNamedImports();
          hasUseImport = namedImports.some(
            (named) => named.getName() === "use",
          );
...
function transformToTailwindClasses(
  sourceFile: SourceFile,
  context: ComponentContext,
): boolean {
  ...
  let newText = oldText
    ...
    .replace(
      /style={{(\s*?)fontSize:(\s*?)['"](\d+)px['"]/g,
      'className="text-$3"',
    )
    .replace(
      /style={{(\s*?)fontWeight:(\s*?)['"]bold['"]/g,
      'className="font-bold"',
    )
    .replace(/style={{(\s*?)display:(\s*?)['"]flex['"]/g, 'className="flex"')
    .replace(
      /style={{(\s*?)flexDirection:(\s*?)['"]column['"]/g,
      'className="flex flex-col"',
    )
    .replace(
      /style={{(\s*?)alignItems:(\s*?)['"]center['"]/g,
      'className="items-center"',
    )
    .replace(
      /style={{(\s*?)justifyContent:(\s*?)['"]center['"]/g,
    ...
    if (initializer && Node.isStringLiteral(initializer)) {
      ...
      if (classNames.length > 50 && !attr.getText().includes("cn(")) {
        ...
        const layoutClasses = classes.filter((c) =>
          c.match(
            /^(flex|grid|block|inline|hidden|overflow|position|inset|top|right|bottom|left|z|float|clear)/,
          ),
        );
        ...
        const typographyClasses = classes.filter((c) =>
          c.match(
            /^(text|font|tracking|leading|list|whitespace|break|hyphens)/,
          ),
        );

        const visualClasses = classes.filter((c) =>
          c.match(
            /^(bg|text-[a-z]|border|rounded|shadow|opacity|ring|outline|fill|stroke)/,
          ),
        );

        const interactionClasses = classes.filter((c) =>
          c.match(
            /^(hover|focus|active|disabled|group|peer|visited|focus-within|focus-visible)/,
          ),
        );
        ...
        const suggestion =
          '// Consider using cn() utility: import { cn } from "@/utils/cn";';
...
function addDarkModeVariants(
  ...
  const jsxAttributes = sourceFile
    ...
    .filter((attr) => {
      ...
      const hasColorClasses = attr
        .getText()
        .match(
          /(bg|text|border)-(white|black|gray|slate|blue|red|green|yellow|purple|pink|indigo)/,
        );
  ...
  for (const attr of jsxAttributes) {
    ...
    if (initializer && Node.isStringLiteral(initializer)) {
      ...
      const colorClassesRegex = new RegExp(
        /(bg|text|border)-(white|black|gray|slate|blue|red|green|yellow|purple|pink|indigo)(-\d+)?/g,
      );
  ...
  for (const arrowFunc of arrowFunctions) {
    ...
    for (const param of parameters) {
      if (Node.isParameterDeclaration(param) && param.isParameterProperty()) {
        const bindingPattern = param.getFirstDescendantByKind(
