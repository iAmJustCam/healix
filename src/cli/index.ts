...
function displayBanner(): void {
  ...
  console.log(
    chalk.cyan("Modern codebase healing system - v0.1.0\n"),
  );
}

/**
 * Loads configuration from healix.config.js if available
 */
function loadConfig(configPath: string = "./healix.config.js"): any {
  ...
  if (fs.existsSync(absolutePath)) {
    try {
      ...
      console.warn(
        chalk.yellow("Failed to load configuration file, using defaults."),
      );
      return {};
    }
  }
...
program
  .name("healix")
  .description(
    "Modern codebase healing system for React 19, Next.js 15, TypeScript 5, and Tailwind v4",
  )
  .version("0.1.0");
...
program
  ...
  .option(
    "--focus <area>",
    "Focus analysis on specific area (react, nextjs, typescript, tailwind, tests)",
  )
  .option(
    "--summary",
    "Show summary of rule coverage instead of detailed report",
  )
  .option(
    "--config <path>",
...
program
  ...
  .action(async (options) => {
    ...
    try {
      ...
      const healingReport = await healProject(
        context,
        report.results,
        healingOptions,
      );
      ...
      if (healingReport.stats.limitApplied) {
        console.log(
          chalk.yellow(
            `⚠️  FIX LIMIT APPLIED - Limited to ${healingReport.stats.fixLimit} fixes (${healingReport.stats.remainingIssues} issues remain)`,
          ),
        );
      }

      // Count operations by type
      const operationsByType = healingReport.operations.reduce<
      ...
      Object.entries(operationsByType).forEach(([type, count]) => {
        console.log(
          `  ${chalk.green("✓")} ${chalk.bold(type)}: ${count} operations`,
        );
      });
      ...
      const uniqueFilePaths = [
        ...new Set(
          healingReport.operations.filter((op) => op.path).map((op) => op.path),
        ),
      ];
      ...
      console.log("\n" + chalk.cyan("📊 Summary:"));
      console.log(
        `  ${chalk.bold("Total issues")}: ${healingReport.stats.totalIssues}`,
      );
      console.log(
        `  ${chalk.bold("Fixed issues")}: ${healingReport.stats.fixedIssues}`,
      );

      if (healingReport.stats.limitApplied) {
        console.log(
          `  ${chalk.bold("Remaining issues")}: ${healingReport.stats.remainingIssues}`,
        );
      }
      ...
      if (options.generateTests) {
        ...
        console.log(
          `  ${chalk.green("✓")} Generated tests for ${uniqueFilePaths.length} components`,
        );
      }
    } catch (error) {
...
program
  ...
  .description(
    "Generate context-aware Vitest tests for components, hooks, and utilities",
  )
  .option("-d, --dir <directory>", "Project directory", ".")
  ...
  .option(
    "--dry-run",
    "Show which tests would be generated without writing files",
    false,
  )
  ...
  .action(async (options) => {
    ...
    try {
      ...
      // Show summary
      console.log(
        `  ${chalk.green("✓")} Processed ${testReport.summary.total} files`,
      );
      console.log(
        `  ${chalk.green("✓")} Created ${testReport.summary.created} new test files`,
      );
      console.log(
        `  ${chalk.yellow("⚠")} Skipped ${testReport.summary.skipped} existing files`,
      );

      // Display by file type
      console.log("\nTests by file type:");
      console.log(
        `  ${chalk.green("✓")} ${chalk.bold("Components")}: ${testReport.summary.componentTests}`,
      );
      console.log(
        `  ${chalk.green("✓")} ${chalk.bold("Hooks")}: ${testReport.summary.hookTests}`,
      );
      console.log(
        `  ${chalk.green("✓")} ${chalk.bold("Utilities")}: ${testReport.summary.utilityTests}`,
      );

      // Group by operation type
      const testsByType = testReport.operations.reduce(
        (acc: Record<string, number>, op: any) => {
      ...
      const createdTests = testReport.operations.filter(
        (op) => op.success && op.testPath,
      );
      if (createdTests.length > 0) {
        console.log("\nExample tests created:");
        createdTests
          .slice(0, Math.min(5, createdTests.length))
          .forEach((op) => {
            console.log(
              `  ${chalk.green("✓")} ${path.relative(options.dir, op.testPath || "")} (${op.entityKind || "unknown"})`,
            );
          });

        if (createdTests.length > 5) {
          console.log(
            `  ${chalk.gray("... and")} ${createdTests.length - 5} ${chalk.gray("more")}`,
          );
        }
      }
      ...
      if (options.dryRun) {
        ...
        console.log(
          chalk.yellow("   Run without --dry-run to create the test files"),
        );
      }
    } catch (error) {
...
program
  ...
  .action(async (options) => {
    ...
    try {
      ...
      const { validateTypeScriptBestPractices } = await import(
        "../validators/typescript-best-practices-validator"
      );
      ...
      if (bestPracticesResults.issues.length === 0) {
        console.log(
          chalk.green("✨ No TypeScript best practice issues found!"),
        );
      } else {
        console.log(
          chalk.yellow(
            `Found ${bestPracticesResults.issues.length} TypeScript best practice issues`,
          ),
        );

        // Group issues by type
        const issuesByCode = bestPracticesResults.issues.reduce(
          (acc: Record<string, ValidationIssue[]>, issue) => {
        ...
        Object.entries(issuesByCode).forEach(([code, issues]) => {
          ...
          issues.slice(0, 3).forEach((issue) => {
            console.log(
              `  ${issue.type === "error" ? chalk.red("✗") : chalk.yellow("⚠")} ${issue.message} (${issue.filePath}:${issue.line})`,
            );
          });
          if (issues.length > 3) {
            console.log(
              chalk.gray(`  ... and ${issues.length - 3} more similar issues`),
            );
          }
        });
        ...
        if (options.fix) {
          ...
          console.log(
            chalk.yellow(
              "Automatic fixing is not yet implemented for all issues",
            ),
          );
          console.log(
            chalk.cyan(
              "💡 See documentation in /docs/typescript-error-prevention.md for manual fixes",
            ),
          );
        } else {
          console.log(
          ...
          console.log(
            chalk.cyan(
              "📖 See documentation in /docs/typescript-error-prevention.md for details",
            ),
          );
        }
      }
...
function displaySummary(report: AnalysisReport) {
  ...
  console.log(
    `  ${chalk.blue("ℹ")} ${chalk.bold(suggestionCount)} suggestions`,
  );
  ...
  if (errorCount + warningCount + suggestionCount === 0) {
    console.log(
      "\n" +
        chalk.green(
          "✨ No issues found! Your project is aligned with best practices.",
        ),
    );
  } else {
    console.log("\n" + chalk.cyan("💡 Next steps:"));
    console.log(
      `  Run ${chalk.bold("healix heal")} to automatically fix detected issues`,
    );
    console.log(
      `  Or run ${chalk.bold("healix heal --dry-run")} to preview changes first`,
    );
  }
}
...
function countIssuesBySeverity(
  ...
  // Project issues
  count += report.results.project.issues.filter(
    (i) => i.type === severity,
  ).length;
  ...
  // Component issues
  count += report.results.components.issues.filter(
    (i) => i.type === severity,
  ).length;

  // Page issues
  count += report.results.pages.issues.filter(
    (i) => i.type === severity,
  ).length;

  // Hook issues
  count += report.results.hooks.issues.filter(
    (i) => i.type === severity,
  ).length;

  // Utility issues
  count += report.results.utilities.issues.filter(
    (i) => i.type === severity,
  ).length;
...
function displayRuleCoverageSummary(report: AnalysisReport) {
  ...
  Object.entries(issuesByFramework).forEach(([framework, count]) => {
    if (
      framework !== "other" &&
      (count > 0 || filesByFramework[framework] > 0)
    ) {
      console.log(
      ...
      const frameworkRules = Object.entries(issuesByRuleCode)
        .filter(([code, issues]) =>
          issues.some((issue) => issue.framework === framework),
        )
        .map(([code, issues]) => {
          const frameworkIssues = issues.filter(
            (issue) => issue.framework === framework,
          );
          return {
            code,
            count: frameworkIssues.length,
            errorCount: frameworkIssues.filter((i) => i.type === "error")
              .length,
            warningCount: frameworkIssues.filter((i) => i.type === "warning")
              .length,
            suggestionCount: frameworkIssues.filter(
              (i) => i.type === "suggestion",
            ).length,
          };
        })
 ...
 */
function groupIssuesByRuleCode(
  report: AnalysisReport,
  ...
  Object.values(report.results.frameworks).forEach((framework) =>
    processIssues(framework.issues),
  );
...
function countFilesByFramework(report: AnalysisReport): Record<string, number> {
  ...
  const counts = {
    react:
      report.results.components.issues.length > 0
        ? [...new Set(report.results.components.issues.map((i) => i.filePath))]
            .length
        : 0,
    nextjs:
      report.results.frameworks.nextjs.issues.length > 0
        ? [
            ...new Set(
              report.results.frameworks.nextjs.issues.map((i) => i.filePath),
            ),
          ].length
        : 0,
    typescript:
      report.results.frameworks.typescript.issues.length > 0
        ? [
            ...new Set(
              report.results.frameworks.typescript.issues.map(
                (i) => i.filePath,
              ),
            ),
          ].length
        : 0,
    tailwind:
      report.results.frameworks.tailwind.issues.length > 0
        ? [
            ...new Set(
              report.results.frameworks.tailwind.issues.map((i) => i.filePath),
            ),
          ].length
        : 0,
    vitest:
      report.results.frameworks.vitest.issues.length > 0
        ? [
            ...new Set(
              report.results.frameworks.vitest.issues.map((i) => i.filePath),
            ),
          ].length
        : 0,
    architecture: 0,
  ...
  Object.keys(counts).forEach((key) => {
    if (
      report.results.frameworks[key as keyof typeof report.results.frameworks]
        ?.issues.length > 0 &&
  ...
  function addIssues(issues: ValidationIssue[]) {
    issues.forEach((issue) => {
      ...
      counts[framework as keyof typeof counts] =
        (counts[framework as keyof typeof counts] || 0) + 1;
    });
  }
